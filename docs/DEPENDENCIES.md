...
`crates/bluetemp-ota/src/ota_record.rs` implements only the two-slot selection record, CRC, and next-sector choice. It uses the actual booted partition from the official API, rejects invalid CRC/aborted records, preserves the running slot's newest record, and refuses sequence overflow. Record format and CRC were checked against Espressif's [bootloader selection code](https://github.com/espressif/esp-idf/blob/v5.5.1/components/bootloader_support/src/bootloader_common_loader.c). Host tests cover erased data, repeated A/B updates, CRC corruption, torn records and sequence exhaustion. Power-failure behavior on real flash remains untested.

Host coverage now executes the shared OTA implementation using `embedded-storage 0.3.1` and the official bootloader crate's `std` support. `md-5 0.10.6` validates test partition tables, and Embassy's built-in mock time driver advances deadlines. These are pure-Rust host test dependencies/features. Target builds enable the official `FlashRegion` storage-trait implementations; the permitted device C/binary components remain as listed above.
