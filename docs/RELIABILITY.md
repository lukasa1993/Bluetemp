...
Host tests exercise the actual community watchdog core with a simulated clock/hardware: initial grace, each task independently missing its deadline, and healthy progress over ten simulated minutes. The actual Embassy clock/ticker and starvation latch are additionally exercised through Embassy's `MockDriver`, including late progress after a latched fault. Shared connection tests cover accept failure/timeouts and a stalled request reaching its absolute deadline. Sensor tests exercise read and soft-reset timeouts, error publication and recovery. Shared OTA tests cover read/write faults, corrupt readback, authentication, exact partition geometry, truncated transfer without metadata commit, retry/cancellation, concurrent updates and post-commit exclusion.

These host checks do **not** test the RTC peripheral, physical flash power loss, actual interrupt/executor stalls, two on-device HTTP workers, electrical I²C recovery, Ethernet negotiation on the wire or power supply behavior. Hardware acceptance remains required.
